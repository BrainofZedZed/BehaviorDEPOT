...
function out = filterIntersect(Behavior, Behavior_Filter, Params)
    ...
    num_events = length(fieldnames(Behavior_Filter.Temporal)); % ZZ added 6/29/21
    ...
    for i = 1:length(beh_cell)   % loop through behaviors
        ...
        for j = 1:Params.num_roi % loop through ROIs
            ...
            for k = 1:num_events  % loop through events; replaced Params.num_events with num_events
